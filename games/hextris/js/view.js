// Local adaptation, 2026-09-05. See LOCAL-CHANGES.md. GPL-3.0-or-later.
...
function drawScoreboard() {
	...
	if (gameState === 0) {
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy, 60, "rgb(236, 240, 241)", '▶', 'px sans-serif');
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2.1 + gdy - 155 * settings.scale, 150, "#2c3e50", "Hextris");
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h + 10, fontSize, "rgb(44,62,80)", '点此开始');
	} else if (gameState != 0 && textOpacity > 0) {
		textOpacity -= 0.05;
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy, 60, "rgb(236, 240, 241)", '▶', 'px sans-serif');
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy - 155 * settings.scale, 150, "#2c3e50", "Hextris");
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h, fontSize, "rgb(44,62,80)", '点此开始');
...
function showText(text) {
	var messages = {
		'paused': "<div class='centeredHeader unselectable'>已暂停</div><div class='local-pause-hint'>点下方「继续」回到游戏</div>",
		'pausedAndroid': "<div class='centeredHeader unselectable'>已暂停</div><div class='local-pause-hint'>点下方「继续」回到游戏</div>",
		'pausediOS': "<div class='centeredHeader unselectable'>已暂停</div><div class='local-pause-hint'>点下方「继续」回到游戏</div>",
		'pausedOther': "<div class='centeredHeader unselectable'>已暂停</div><div class='local-pause-hint'>点下方「继续」回到游戏</div>",
