...
function initialize(a) {
	...
	window.op = 0;
	window.saveState = localStorage.getItem("pocket.hextris.saveState") || "{}";
	if (saveState !== "{}") {
		op = 1;
	}
	...
	highscores = [];
	if (localStorage.getItem('pocket.hextris.highscores')) {
		try {
			highscores = JSON.parse(localStorage.getItem('pocket.hextris.highscores'));
		} catch (e) {
			highscores = [];
		}
	}
	...
	if (a != 1) {
		...
		$(window).unload(function() {
			...
			if (gameState == 1 || gameState == -1 || gameState === 0) localStorage.setItem("pocket.hextris.saveState", exportSaveState());
			else localStorage.setItem("pocket.hextris.saveState", "{}");
		});
		...
		setTimeout(function() {
			if (settings.platform == "mobile") {
				try {
					canvas.removeEventListener('touchstart', handleTapBefore, false);
				} catch (e) {

				}

				try {
					canvas.removeEventListener('touchstart', handleTap, false);
				} catch (e) {

				}

				canvas.addEventListener('touchstart', handleTapBefore, false);
			} else {
				try {
					canvas.removeEventListener('mousedown', handleClickBefore, false);
				} catch (e) {
				...
				canvas.addEventListener('mousedown', handleClickBefore, false);
			}
		}, 1);
	}
}
