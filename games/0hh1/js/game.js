...
var Game = new (function() {
  var self = this,
      ...
      currentPuzzle = null,
      checkTOH = 0,
      ojoos = ['解开了','思路清晰','完成一局','恰到好处','漂亮的推理'],
      remainingOjoos = [],
      endGameTOH1,
      endGameTOH2,
      endGameTOH3,
  ...
  function tutorialPlayed() {
    ...
    return (window.localStorage.getItem('shunsui-0hh1-tutorialPlayed') + '') == 'true';
  }

  function markTutorialAsPlayed() {
    if (!window.localStorage) return;
    window.localStorage.setItem('shunsui-0hh1-tutorialPlayed', true);
  }
  ...
  function getScore() {
    return (window.localStorage.getItem('shunsui-0hh1-score') * 1);
  }
  ...
  function setScore(addPoints) {
    ...
    window.localStorage.setItem('shunsui-0hh1-score', newScore);
    return newScore;
  }
  ...
  function undo() {
    if (!undoStack.length) {
      ...
      if (!undone)
        grid.hint.show('这里可以撤销上一步。');
      else
        grid.hint.show('还没有可以撤销的步骤。');
      return;
    }
  ...
  function updateTweetUrl(size) { /* Sharing disabled in local adaptation. */ }
